...
function findHebrewDictionaryEntries(rawHebrew) {
    ...
    const normalized = normalizeHebrewLemmaForLookup(rawHebrew);
    const compact = normalized.replace(/\s+/g, '');
    if (!compact) return [];

    const idList = index[compact] || index[normalized] || [];
    ...
    const exact = source.filter(entry => normalizeHebrewLemmaForLookup(entry.lemma).replace(/\s+/g, '') === compact);
    if (exact.length) return exact;

    const headwordHits = source.filter(entry => {
        const lemma = normalizeHebrewLemmaForLookup(entry.lemma).replace(/\s+/g, '');
        if (lemma === compact) return true;
        const headword = normalizeHebrewLemmaForLookup(entry.headword_line).replace(/\s+/g, '');
        if (headword.startsWith(compact) || headword.includes(compact)) return true;
        return Array.isArray(entry.headword_tokens) && entry.headword_tokens.some(token => normalizeHebrewLemmaForLookup(token).replace(/\s+/g, '') === compact);
    });
