(() => {
  ...
  function expandMorphologyCandidates(input){
    ...
    const push = (x) => { 
      if (x && /[א-ת]/.test(x)) {
        ...
        // NUEVO: Inteligencia de Ortografía Plena vs Defectiva en la raíz resultante
        if (fixed.length >= 4) {
          // Si tiene Yod o Vav penúltima (Ej: מליכ -> מלכ -> מלך)
        ...
        // NUEVO: Inteligencia para Participios Activos (Qal Qotel - Ej: עובר -> עבר, שופט -> שפט)
    ...
    // 1. Sufijos pronominales y verbales unidos (Incluye los nuevos: ני, הו, מו, נה)
    ...
    // 3. NUEVO: Terminación Aramea Definitiva (Alef en lugar de He. Ej: מלכותא -> מלכות)
  ...
  function generatePureWordCandidates(tok, depth = 0, seen = new Set()){
    const IRREGULARS = {
      ...
      "נורא": "ירא", "והנורא": "ירא",
      "מגיע": "נגע",
      // NUEVOS IRREGULARES EXTREMOS de tu lista
      "פה": "פה", "פיהו": "פה", "פי": "פה", "פיו": "פה", "פיך": "פה", "פיהם": "פה", // Boca
      "ויעש": "עשה", "ותעש": "עשה", "יעש": "עשה", "תעש": "עשה", "אעש": "עשה", "נעש": "עשה", // Verbo hacer truncado
      "וירא": "ראה", "ירא": "ראה", "תרא": "ראה", "ארא": "ראה", "נרא": "ראה", // Verbo ver truncado
      "ויצו": "צוה", "יצו": "צוה", "ויבך": "בכה", "יבך": "בכה" // Mandar y llorar
    };
  ...
  function searchHebrewWordSingle(rawInput) {
    ...
    // Filtro 1a: exacto literal de la forma consultada (prioridad máxima).
    // Si existe la forma vocalizada exacta, NO mezclar con variantes sin vocales.
    ...
    // Filtro 3 estricto: morfología de alta precisión (sin expandir a raíces vecinas por recursión amplia)
    ...
    // Filtro 4: consonántico, pero restringido a candidatos morfológicos precisos (evita ruido de raíces vecinas)
    ...
    return { ok:false, tier:'Sin resultados', matches:[], trace, diag:'No hubo coincidencias tras pasar los 4 filtros.' };
  }
  ...
  // Lista ampliable de compuestos lexicalizados (sin guion) que conviene dividir por partes.
  // Clave y partes se comparan sin vocales/niqqud/cantillación.
  ...
  function findLexicalizedCompoundPlans(rawSpan) {
    ...
    // 2) Heurística: si el término completo NO existe como entrada sin vocales, intentar segmentación.
    //    (evita falsos positivos en palabras simples que por casualidad se puedan cortar)
    ...
    for (let i = 2; i <= wholeKey.length - 2; i++) {
      ...
      if (wholeExists && !known.length) continue; // conservador si ya existe como unidad
      const parts = [a, b];
      plans.push({ kind:'heuristic', parts, score: scoreCompoundSplit(parts, wholeKey) });
    }

    // Heurística 3 partes (solo si no hay plan conocido y la palabra es larga)
  ...
  function resolveBySeparatedParts(parts, rawSpan, label, options = {}) {
    const requireAllParts = options.requireAllParts !== false; // por defecto sí
    const allMatches = [];
    const partsTrace = [];
    const partsFound = [];
    const partsMissing = [];
    ...
    const trace = [
      ...
      'Política aplicada: devolver resultados por palabra/raíz separada (no como unidad compuesta).',
    ...
    return {
      ok: true,
      tier: label.includes('lexicalizado') ? 'Compuesto lexicalizado dividido: resultados por partes' : 'Compuesto dividido: resultados por partes',
      matches: merged,
      trace,
      diag: `Se detectó compuesto${label.includes('lexicalizado') ? ' lexicalizado ' : ' '}y se buscaron ${parts.length} componente(s) por separado (hebreo + español).`
    };
  }
  ...
  function searchHebrewWord(rawInput) {
    ...
    // Si la consulta trae guion/maqaf explícito (ej. בית-ספר), buscar por PARTES, no como unidad.
    if (hasCompoundConnector(rawSpan) && q.tokens.length >= 2) {
      const resParts = resolveBySeparatedParts(q.tokens, rawSpan, 'Consulta compuesta detectada (guion/maqaf)', { requireAllParts:false });
      if (resParts) return resParts;

      // Si ninguna parte resolvió, se intenta la búsqueda normal completa como respaldo.
      const fallback = searchHebrewWordSingle(rawInput);
      fallback.trace = [
        `Consulta compuesta detectada (guion/maqaf): ${rawSpan}`,
        `Se intentó por partes sin éxito: ${q.tokens.join(' + ')}`,
    ...
    // Compuestos lexicalizados SIN guion (ej. בארשבע): dividir por partes si hay plan conocido
    // o una segmentación heurística de alta confianza.
  ...
  function sourceCategory(source) {
    ...
    if (/gen|exo|lev|num|deut|torah|pentateuco/.test(s)) return 'Torah';
    if (/jos|jue|sam|rey|cron|isa|jer|eze|ose|amos|miqueas|profet/.test(s)) return 'Profetas';
    if (/mate|marc|luc|juan|hech|evangel/.test(s)) return 'Evangelios';
    if (/rom|cor|gal|efe|fil|col|tes|tim|tit|flm|heb|sant|ped|jud|apoc|cartas/.test(s)) return 'Cartas';
    return 'Corpus';
  }
  ...
  function renderDeepAnalysis(res, rawQuery) {
    ...
    const cards = [
      ...
      { label:'Coincidencia principal', value:first?.he || '—' },
      { label:'Glosa principal', value:first?.es || '—' },
  ...
  function updateAnalysisPanels(res, rawQuery) {
    ...
    if (lemmaSummaryEl) {
      lemmaSummaryEl.textContent = first
        ? `La búsqueda se resolvió con ${matches.length} coincidencia(s). La forma principal es ${first.he} y su glosa candidata es ${first.es || '—'}.`
        : 'No se encontraron coincidencias para la consulta actual.';
    }
    if (deepLexicalCorrespondenceEl) {
      const firstEs = first?.es || '—';
      deepLexicalCorrespondenceEl.textContent = `Correspondencias idiomáticas: Hebreo ${first?.he || '—'} / Español ${firstEs} / Filtro ${res?.tier || '—'}`;
    }
