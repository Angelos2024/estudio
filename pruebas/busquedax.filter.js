...
;(() => {
  ...
  function _splitCandidateTerms(value) {
    if (!value) return [];
    if (Array.isArray(value)) return value.flatMap(_splitCandidateTerms);
    return String(value)
      .split(/[\/,;|]+/g)
      .map((part) => part.trim())
      .filter(Boolean);
  ...
  function _ingestPruebasRows(rows) {
    if (!Array.isArray(rows)) return;
    for (const row of rows) {
      if (!row || typeof row !== 'object') continue;
      const heList = _splitCandidateTerms(row.texto_hebreo || row.he || row.hebreo);
      const grList = _splitCandidateTerms(row.equivalencia_griega || row.gr || row.griego);
      const esList = [
        ..._splitCandidateTerms(row.equivalencia_espanol || row.es || row.espanol),
        ..._splitCandidateTerms(row.candidatos || row.alias || row.sinonimos)
      ];
  ...
  async function _loadPruebasEquivalences(options = {}) {
    const files = Array.isArray(PRUEBAS_DICT_FILES) ? PRUEBAS_DICT_FILES : [];
    if (!files.length) return false;

    let loadedAny = false;
    for (const file of files) {
      try {
        const rows = await loadJson(file, options);
        _ingestPruebasRows(rows);
        loadedAny = true;
      } catch (err) {
        console.warn('[busquedapruebas] No se pudo cargar diccionario de pruebas:', file, err);
      }
    }
    return loadedAny;
  }
  ...
  async function loadTrilingualEquivalences(options = {}) {
    ...
    TRI.loading = (async () => {
      let data = null;
      try {
        data = await loadJson(TRILINGUAL_EQUIV_URL, options);
      } catch (err) {
        console.warn('[busquedapruebas] Equivalencias trilingües no disponibles, se usará diccionario de pruebas.', err);
      }

      if (!data) {
        const loadedFromPruebas = await _loadPruebasEquivalences(options);
        TRI.loaded = true;
        return TRI;
      }
